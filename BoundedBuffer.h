...
private:
	...
	std::mutex mtx; // Synchronizing access to the queue, and prevents race conditions
	std::condition_variable data_available; // size < cap, prevents overflow (push)
	std::condition_variable slot_available; // size > 0, prevents underflow (pop)
